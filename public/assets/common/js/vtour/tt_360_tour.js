...
window.TT_VARS = {
  dev: {
    TT_HOST_URL:
      ...
      "https://www-tt.touristtube.com/",
    TT_TOUR_XML_URL:
      "https://utildev01-gcp.touristtube.com:8765/media/360-photos-vr/vtourConfig/",
  },
  prod: {
    TT_HOST_URL: "https://www.touristtube.com/",
    TT_TOUR_XML_URL:
      "https://api_gw_service.touristtube.com:8765/media/360-photos-vr/vtourConfig/",
...
function TTTour(_id, _options, _win, _doc) {
  ...
  if (!_id || _id.split(" ").join("") == "")
    throw "Provided DOM selector cannot be empty or null";
  ...
  //
  var TOUR_HOST_URL =
    objThis.win.TT_VARS[objThis.win.TT_TOUR_ENV]["TT_HOST_URL"];
  var TOUR_XML_URL =
    objThis.win.TT_VARS[objThis.win.TT_TOUR_ENV]["TT_TOUR_XML_URL"];
  ...
  function _loadJS() {
    ...
    if (!objThis.win.embedpano) {
      if (!objThis.win.TT_VIEWER_LOADED && !objThis.win.TT_VIEWER_LOADING) {
        ...
        //
        objThis.JQ.getScript(TOUR_JS_URL)
          .done(function (data, textStatus, jqxhr) {
            ...
            if (!objThis.win.embedpano) {
              console.error(
                "Touristtube 360 Tour library is not properly loaded."
              );
            } else {
      ...
      } else if (objThis.win.TT_VIEWER_LOADING)
        objThis.win.setTimeout(_loadJS, 500);
  ...
  function _instantiateFullTourModal() {
    ...
    if (fullToorOptions.active || fullToorOptions.linkOrigin) {
      ...
      if (fullToorOptions.openTarget != "popup") {
        ...
        if (
          objThis.options.loadPopupLibrary &&
          !objThis.TT_POPUP_LIBRARY_LOADED
        ) {
          objThis
            .JQ("head")
            .append(
  ...
  function _instantiateFullTour(linkOrigin, fullTourURL) {
    ...
    modalName += encodeURIComponent(
      objThis.options.params.name.split(" ").join("_") || ""
    );
    ...
    if (objThis.options.params.subdivision || objThis.options.params.division)
      modalName +=
        "_" +
        (objThis.options.params.subdivision || objThis.options.params.division);
    //
    ...
    objThis
      ...
      .attr(
        "data-options",
        '{"iframe" : {"css" : {"width" : "' +
          fullToorOptions.width +
          '", "height" : "' +
          fullToorOptions.height +
          '"}}}'
      );
    ...
    if (objThis.COUNT_AJAX_XML <= 3) {
      ...
      objThis.JQ.ajax(xmlURL, {
        ...
        success: function (data, textStatus, jqXHR) {
          ...
          //console.log(objThis.options.params.showThumbs);
          var xmlstring = data;

          if (objThis.options.params.webvr === false)
            xmlstring = xmlstring.replace('webvr="true"', 'webvr="false"');

          if (objThis.options.params.showThumbs === false) {
          ...
          objThis.LIB_INSTANCE.call(
            "loadxml(" + xmlstring + ", null, null, BLEND(0.5));"
          );
          objThis.COUNT_AJAX_XML = 0;
        },
        error: function (jqXHR, textStatus, errorThrown) {
          //
          console.log(
            "TTVTour ERROR: Ajax failed to retrieve XML config [" + xmlURL + "]"
          );
  ...
  function _prepareQueryParams(skipFirstParamSeparator) {
    var params = "";
    console.log(objThis.options.params);
    if (objThis.options.params) {
      for (var pname in objThis.options.params) {
        //console.log(pname);
        //console.log(objThis.options.params[pname]);
        // if(!objThis.options.params[pname]) continue;
        if (
          typeof objThis.options.params[pname] === "undefined" ||
          objThis.options.params[pname] == null
        )
          continue;
    ...
    //
    //return params+"&showThumbs=false";
    return params;
  }
  ...
  function _preparePathVariables(subDivisionId, divisionId, entityId) {
    ...
    if (objThis.options.params) {
      ...
      if (paramObj.group) params.push(paramObj.group);
  ...
  function _loadMainThumbnail() {
    if (objThis.options.showThumbnail) {
      ...
      //
      objThis.selectorElt.prepend(
        "<img src='" +
          TT_TOUR_THUMB_URL +
          "' onerror='this.style.display=\"none\";' class='touristtube_360_thumb' style='max-width: 100%; max-height: 100%;'/>"
      );
      //
      if (!objThis.options.autoload) {
        objThis.selectorElt
          .find("img.touristtube_360_thumb")
          .on("click", function () {
            objThis.loadEngine();
          });
      }
  ...
  function _initHeaders(headerTags) {
    if ((TT_TOUR_HEADER && !objThis.win.HEADER_INITIALIZED) || headerTags) {
      ...
      for (var i = 0; i < headerTags.length; i++) {
        var tag = objThis.JQ(
          "<" + headerTags[i].tag + "></" + headerTags[i].tag + ">"
        );
        var oldTag = objThis.JQ(
          "head " +
            headerTags[i].tag +
            "[name='" +
            headerTags[i].attributes.name +
            "']"
        );
        ...
        for (var attribute in headerTags[i].attributes) {
          if (oldTag) {
            if (attribute.toLowerCase() != "name")
              oldTag.attr(
                attribute,
                oldTag.attr(attribute) +
                  " " +
                  headerTags[i].attributes[attribute]
              );
          } else {
  ...
  function _getPanoramaUrl(subDivisionId, divisionId, entityId) {
    ...
    var entityName = encodeURIComponent(
      objThis.options.params.name.split(" ").join("-")
    );
    //
    var TT_PANORAMA_PATH = null;
    if (objThis.win.TT_PANORAMA_PATH)
      TT_PANORAMA_PATH = objThis.win.TT_PANORAMA_PATH;
    else if (objThis.options.params.subdivision || subDivisionId)
      TT_PANORAMA_PATH = pathVars;
    //
    if (TT_PANORAMA_PATH) {
      TT_PANORAMA_PATH =
        TT_TOUR_PANORAMA_URL +
        TT_PANORAMA_PATH +
        "/" +
        objThis.options.panoramaThumbName +
  ...
  function _initPopupImports() {
    objThis
      ...
      .append(
        '<link rel="stylesheet" href="' +
          TOUR_HOST_URL +
          'assets/common/tt/js/vtour/css/socialshare.css" type="text/css" />'
      );
  ...
  function _showSocialShare(subDivisionId, divisionId, entityId) {
    ...
    if (TT_PANORAMA_PATH) {
      var html =
        '<div class="tt-vtour-social-share">' +
        ...
        TT_FB_APP_ID +
        "&title=tt&display=page&href=" +
        TT_PANORAMA_PATH +
        '&redirect_uri=" target="_blank" rel="nofollow" title="facebook" class="fb-share sharecommun"></a>' +
        // + ' <a
        ...
        TT_PANORAMA_PATH +
        '" target="_blank" rel="nofollow" title="google+" class="g-share sharecommun"></a>' +
        '			<a href="http://www.twitter.com/intent/tweet?url=' +
        TT_PANORAMA_PATH +
        '" target="_blank" rel="nofollow" title="twitter" class="twt-share sharecommun"></a>' +
        '			<a href="https://pinterest.com/pin/create/bookmarklet/?media=' +
        TT_PANORAMA_PATH +
        "&url=" +
        TT_PANORAMA_PATH +
        '" target="_blank" rel="nofollow" title="pinterest" class="print-share sharecommun"></a>' +
        "		</div>" +
        "	</div>" +
        "	</div>" +
        "</div>";
      //
      //
      if (objThis.options.loadPopupLibrary)
        objThis.JQ.fancybox.open(objThis.JQ(html), { type: "iframe" });
  ...
  function _initCallBacks() {
    ...
    objThis.win.ON_TT_VTOUR_SCENE_STARTED = function (
      subDivisionId,
      divisionId
    ) {
      objThis.win.TT_VTOUR_LOADED_SCENE = {
    ...
    objThis.win.TT_VTOUR_SHARE_ACTION = function () {
      ...
      if (instance)
        instance.shareTour(
          TT_VTOUR_LOADED_SCENE.subDivisionId,
          TT_VTOUR_LOADED_SCENE.divisionId
        );
    };
  }
  ...
  function _init() {
    ...
    TT_TOUR_THUMB_URL +=
      pathVars +
      objThis.options.thumbName +
      objThis.options.thumbExtension +
      "/" +
      entityName +
      TT_TOUR_THUMB_SUFFIX +
      objThis.options.thumbExtension; // objThis.options.params.type
    // +
    // "/"
    // +
    // entityName
  ...
  function _prepareTTInfo() {
    ...
    if (!objThis.win.rePositionInfoPopup) {
      objThis.win.rePositionInfoPopup = function () {
        if (
          $(INFO_CONTAINER_ID).length > 0 &&
          !$(INFO_CONTAINER_ID).hasClass("hide")
        ) {
    ...
    if (!objThis.win.ttInfoAction) {
      objThis.win.ttInfoAction = function () {
        ...
        if ($(INFO_CONTAINER_ID).length == 0) {
          var infoContent = $("<div>")
            .attr("id", INFO_CONTAINER_ID.split("#").join(""))
            .addClass("tt_vtour_info_container hide");
          if (objThis.win.TT_VTOUR_INFO)
            infoContent.html(objThis.win.TT_VTOUR_INFO);
        ...
        var indx =
          $(viewerContainerElt.find("> div").get(3)).find("> div").length == 1
            ? 4
            : 3;
        ...
        var parentLeft = viewerContainerElt
          .parent()
          .parent()
          .parent()
          .position().left;
